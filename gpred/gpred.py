...
def find_start(start_regex, sequence, start, stop):
    ...
    """
    pos_codon = start_regex.search(sequence, start, stop)
    if not pos_codon:
        return None
...
def has_shine_dalgarno(shine_regex, sequence, start, max_shine_dalgarno_distance):
    ...
    """
    codon = shine_regex.search(sequence, start - max_shine_dalgarno_distance)
    if codon:
        if codon.end(0) < start - 6:
            return True
    return False
...
def predict_genes(sequence, start_regex, stop_regex, shine_regex,
    ...
    while len(sequence) - current_pos >= min_gap:
        ...
        if current_pos is not None:
            ...
            if stop is not None:
                gen_len = stop - current_pos
                if gen_len >= min_gene_len:
                    sd = has_shine_dalgarno(shine_regex, sequence, current_pos, max_shine_dalgarno_distance)
                    if sd:
                        gen_pred.append([current_pos + 1, stop + 3])
...
def main():
    ...
    probable_genes = predict_genes(sequence, start_regex, stop_regex, shine_regex, args.min_gene_len,
                                   args.max_shine_dalgarno_distance, args.min_gap)
    # Don't forget to uncomment !!!
    # Call these function in the order that you want
    # We reverse and complement
    sequence_rc = reverse_complement(sequence)
    seq_reverse = predict_genes(sequence_rc, start_regex, stop_regex, shine_regex, args.min_gene_len,
                                args.max_shine_dalgarno_distance, args.min_gap)
    len_seq_rc = len(sequence_rc)
    probable_genes_comp = [[len_seq_rc - codon2, len_seq_rc - codon1] for codon1, codon2 in seq_reverse]
